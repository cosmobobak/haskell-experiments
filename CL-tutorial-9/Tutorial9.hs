...
mkFSM qs as ts ss fs =
  -- a convenience function constructing FSM from lists
...
isDFA (FSM qs as ts ss _) =
  (size ss == 1)
    && and
      [ length [q' | q' <- toList qs, (q, a, q') `member` ts] == 1
        | q <- toList qs,
          a <- toList as
      ]
...
accepts (FSM _ _ ts ss fs) string = (not . null) (fs /\ final)
  where
    final = foldl (transition ts) ss string

-- Q 8.
trace :: Ord q => FSM q -> [Sym] -> [Set q]
trace (FSM qs as ts ss fs) word = reverse (tr ss word)
  where
    tr ss [] = [ss]
...
reverseFSM (FSM qs as ts ss fs) = FSM qs as (fromList (map flipTrans (toList ts))) fs ss
  where
    flipTrans (a, x, b) = (b, x, a)
...
ddelta (FSM _ _ ts _ _) = transition ts
-- applying transitions for a given symbol to move a list of states
