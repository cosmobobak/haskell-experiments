...
somebodyLovesSomeoneWhoLovesThem =
    some people (\x -> some people (\y -> (x `loves` y) && (y `loves` x)))

-- | Exercise 3.2

everybodyLovesSomeoneWhoLovesSomeoneWhoLovesThem :: Bool
everybodyLovesSomeoneWhoLovesSomeoneWhoLovesThem =
...
existsUnique xs p =
    some xs (\x -> p x && every xs (\y -> if p y then x==y else True))
